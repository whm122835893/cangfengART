...
import { Calendar, Tag, Eye, Heart, Share2 } from 'lucide-vue-next'
...
const announcement = computed(() => announcements.find((a) => a.id === Number(id.value)))
</script>

<template>
  <div v-if="!announcement" class="page-container bg-neu-bg">
    <NavBar title="公告详情" />
    <div class="flex flex-col items-center justify-center pt-24">
      <p class="text-lg font-bold text-neu-text-primary">公告不存在</p>
      <button
        class="mt-4 text-accent-blue font-semibold"
        @click="router.back()"
      >
        返回
      </button>
  ...
  <div v-else class="page-container bg-neu-bg">
    ...
    <!-- 封面图 -->
    <div
      class="mx-4 mt-3 h-48 rounded-lg-card neu-raised relative overflow-hidden"
      :style="{ background: announcement.cover }"
    >
      <div class="absolute inset-0 flex items-center justify-center">
        <span class="text-white/20 text-5xl font-black tracking-widest">
          藏锋ART
        </span>
      </div>
      <div class="absolute top-3 left-3">
        <span class="neu-pressed text-white text-xs px-2.5 py-1 rounded-full font-semibold bg-white/10 backdrop-blur-sm">
          {{ announcement.tag }}
        </span>
      </div>
    </div>
    ...
    <!-- 标题 -->
    <div class="px-4 mt-4">
      <h1 class="text-lg font-bold text-neu-text-primary leading-relaxed">
        {{ announcement.title }}
      </h1>

      <!-- 元信息 -->
      <div class="flex items-center gap-4 mt-3 text-xs text-neu-text-muted">
    ...
    <!-- 分隔线 -->
    <div class="neu-divider mx-4 mt-4" />

    <!-- 正文 -->
    <div class="px-4 py-4">
      <div class="text-sm text-neu-text-secondary leading-7 whitespace-pre-line font-medium">
        {{ announcement.content }}
      </div>
    </div>

    <!-- 底部操作 -->
    <div class="flex items-center justify-center gap-6 py-4 mb-4">
      <button class="flex items-center gap-1.5 text-sm text-neu-text-muted font-semibold">
        <Heart :size="18" />
        点赞
      </button>
      <button class="flex items-center gap-1.5 text-sm text-neu-text-muted font-semibold">
