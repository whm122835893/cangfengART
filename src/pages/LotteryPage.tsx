...
export default function LotteryPage() {
  ...
  return (
    <div className="page-container bg-neu-bg pb-4">
      <NavBar title="扭蛋抽奖" />

      {/* 扭蛋机主体 */}
      <div className="flex flex-col items-center mx-4 mt-4">
        <div className="neu-raised rounded-lg-card w-full p-4 flex flex-col items-center relative overflow-hidden"
          style={{ background: 'linear-gradient(180deg, #e8ecf1 0%, #d5dce6 100%)' }}>
          
          {/* ===== 扭蛋机 3D ===== */}
          <div className="relative w-full flex flex-col items-center" style={{ perspective: '800px' }}>
            
            {/* 顶部装饰 / 投币口 */}
            <div className="w-40 h-5 rounded-t-2xl relative z-10"
              style={{ background: 'linear-gradient(180deg, #c0c8d4, #a0aab8)' }}>
              <div className="absolute top-1 left-1/2 -translate-x-1/2 w-8 h-2 rounded-full bg-[#4A90D9] shadow-inner" />
            </div>

            {/* 透明罩 */}
            <div className="relative w-44 h-44 z-0" style={{ transformStyle: 'preserve-3d' }}>
              {/* 罩子主体 */}
              <div className="absolute inset-0 rounded-full overflow-hidden"
                ...
                {/* 高光 */}
                <div className="absolute top-3 left-4 w-10 h-6 rounded-full bg-white/30 rotate-12" />
                <div className="absolute top-6 left-5 w-4 h-2 rounded-full bg-white/40 rotate-12" />
                
                {/* 彩色小球 */}
                <div className={`absolute inset-0 flex items-center justify-center ${shake ? 'animate-pulse' : ''}`}>
                  {ballColors.map((color, i) => (
                    <div
                      key={i}
                      className={`absolute w-7 h-7 rounded-full transition-all duration-200 ${
                        shake ? 'ball-shake' : ''
                      }`}
                      style={{
              ...
              {/* 罩子底部边框 */}
              <div className="absolute bottom-0 left-0 right-0 h-3 rounded-b-full"
                style={{ background: 'linear-gradient(180deg, rgba(180,200,220,0.6), #a0aab8)' }} />
            </div>

            {/* 机器主体 */}
            <div className="w-36 h-20 relative z-[-1] -mt-1"
              style={{
              ...
              {/* 出蛋口 */}
              <div className="absolute bottom-3 left-1/2 -translate-x-1/2 w-10 h-10 rounded-full flex items-center justify-center"
                style={{
                  background: 'radial-gradient(circle, #4a5568 0%, #2d3748 100%)',
                  boxShadow: 'inset 0 2px 4px rgba(0,0,0,0.5), 0 0 0 3px #8892a0',
                }}>
                <div className="w-6 h-6 rounded-full bg-[#2d3748] shadow-inner" />
              </div>
              ...
              <button
                onClick={handleDraw}
                disabled={drawing}
                className="absolute -right-3 top-1/2 -translate-y-1/2 w-14 h-14 rounded-full flex items-center justify-center transition-all duration-200"
                style={{
                  background: 'linear-gradient(145deg, #6DB3F2, #4A90D9)',
                  boxShadow: drawing
                    ? 'inset -1px -1px 2px rgba(255,255,255,0.7), inset 1px 1px 2px rgba(174,186,204,0.5)'
                    : '-1px -1px 2px rgba(255,255,255,0.7), 1px 1px 2px rgba(174,186,204,0.5)',
                  transform: drawing ? 'scale(0.95)' : 'scale(1)',
                }}>
                <span className="text-white text-xs font-bold">{drawing ? '扭动中' : '扭一扭'}</span>
              </button>
            </div>

            {/* 机器底座 */}
            <div className="w-44 h-4 -mt-1"
              style={{
            ...
            {/* 出蛋滑槽 */}
            <div className="w-8 h-6 relative -mt-0.5"
              style={{
            ...
            <div className="w-24 h-7 relative"
              ...
              {ballDropped && result && (
                <div
                  className="absolute -top-5 left-1/2 -translate-x-1/2 w-8 h-8 rounded-full cursor-pointer animate-bounce z-20"
                  style={{
          ...
          {/* 提示文字 */}
          <p className="text-sm text-neu-text-muted font-semibold mt-4">
      ...
      <div className="mx-4 mt-4">
        <div className="flex items-center justify-between mb-3">
          <h3 className="text-sm font-bold text-neu-text-primary">扭蛋记录</h3>
          <button className="flex items-center gap-0.5 text-xs text-neu-text-muted font-semibold">
            查看更多
            <ChevronRight size={14} />
          </button>
        </div>
        <div className="neu-raised rounded-card divide-y divide-neu-divider">
          {[
            ...
            <div key={i} className="p-3 flex items-center gap-3">
              <div className="w-8 h-8 rounded-full neu-accent-blue flex items-center justify-center shrink-0">
              ...
              <span className="text-xs text-accent-blue font-bold neu-pressed px-2 py-0.5 rounded-full shrink-0">
                {item.prize}
              </span>
            </div>
          ))}
      ...
      {showModal && result && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/50"
          ...
          <div className="neu-raised rounded-lg-card mx-6 p-6 flex flex-col items-center w-full max-w-[320px] animate-scale-in"
            onClick={(e) => e.stopPropagation()}>
            {/* 关闭按钮 */}
            <button
              onClick={() => setShowModal(false)}
              className="absolute top-3 right-3 w-7 h-7 neu-raised rounded-full flex items-center justify-center"
            >
              <X size={14} className="text-neu-text-muted" />
            </button>

            {/* 结果图标 */}
            <div className="w-20 h-20 rounded-full neu-inset flex items-center justify-center mb-4">
              <span className="text-5xl">{result.icon}</span>
            </div>

            {/* 结果文字 */}
            <h2 className="text-xl font-bold text-neu-text-primary mb-1">
              {result.name === '谢谢参与' ? '很遗憾' : '恭喜获得'}
            </h2>
            <p className="text-lg font-bold text-accent-blue mb-1">{result.name}</p>
            <p className="text-sm text-neu-text-muted mb-6">{result.desc}</p>

            {/* 按钮 */}
            <div className="flex gap-3 w-full">
              <button
                onClick={() => setShowModal(false)}
                className="flex-1 h-10 neu-raised rounded-2xl text-sm font-bold text-neu-text-primary"
              >
                知道了
              </button>
              <button
                onClick={() => setShowModal(false)}
                className="flex-1 h-10 neu-accent-blue rounded-2xl text-sm font-bold text-white"
              >
                查看详情
              </button>
            </div>
